...
Simple model to predict housing prices using from latitude, longitude, lot area, number of bedrooms, number of bathrooms, along with derived features like floorplan density and distance to the nearest metropolitan area. Built using pytorch, data cleaned and prepped using pandas, numpy. The model is currently at a log-price R^2 value of 0.637 (i.e, my model reduces prediction error variance by 64% over using the mean price to predict).
